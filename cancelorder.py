...
# print(exchange.create_market_sell_order('ADA/USDT',0.09))
# print(exchange.create_market_sell_order('BTC/USDT',0.00000971))
import threading
class binance():
    def ohlcv(symbol):
        usdt_amount=11
    ...
    def PLACEORDER(symbol,KEY,VALUE):
        ...
        # free=amount['free']
        # print(symbol+" "+str(exchange.create_market_sell_order(symbol,free)))
        # binance.cancel(symbol,KEY,VALUE)
        print(str(exchange.fetch_balance()[symbol.split('/')[1]])+" "+symbol)
        # print(str(exchange.fetch_balance()[symbol.split('/')[0]])+" "+symbol)
        # print(symbol+" "+str(exchange.fetch_open_orders(symbol)))
